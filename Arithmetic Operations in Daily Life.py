# Task 1: Grocery Store Math 
# Calculate the total cost of three items you'd commonly find in a grocery store, 
# given their individual prices. 
# For example, what would be the cost of bread, peanut butter, and jelly be? 
# Prices don't need to be realistic!

#Answer:
bread, peanutButter, jelly = 5,3,4
total_grocery_cost = bread + peanutButter + jelly
print(total_grocery_cost)

# Task 2: Bank Interest 
# If you have a savings account with a particular initial amount and a fixed 
# yearly interest rate, calculate the total amount in your account after a year. 
# So if you had $10,000 at a 7% interest write code that would tell us the 
# amount at the end of the year. 
# For the example the expected outcome would be $10,700.

# Answer:
savings_account_balance = 10000
fixed_yearly_interest = 0.07
end_of_year_balance = savings_account_balance * fixed_yearly_interest + savings_account_balance
print(end_of_year_balance)